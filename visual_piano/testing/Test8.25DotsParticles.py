...
def map_midi_velocity_to_intensity(velocity):
    ...
    return int(200 / (1 + math.exp(-(velocity - 64) / 16)))
...
def map_midi_velocity_to_y(midi_value):
    ...
    return int(550 - 500 / (1 + math.exp(-(midi_value - 20) / 16)))
class Particle:
...
class ExpandingDot:
    ...
    def expand(self, velocity):
        self.color = (map_midi_velocity_to_intensity(velocity)+50, map_midi_velocity_to_intensity(velocity)*0.3, 0)
...
if ports: #if midi has port
    ...
    while running:
        ...
        # Clear the screen
        if pedalOn:
            screen.fill((random.randint(70, 90),25,50))
        else:
            screen.fill((0, 0, 0))
